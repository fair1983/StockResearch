...
### 🆕 v0.4.0 更新內容 (2025-08-26)

#### ✨ 全市場掃描器重大優化
- **📈 股票數量提升**: 從200支提升到1000支（美股500 + 台股500）
- **🔄 靈活資料收集**: 新增四種收集模式（test/quick/full/custom）
- **⚡ 效能優化**: 改進API速率限制和批次處理
- **🛡️ 資料安全**: 實作檔案備份和錯誤恢復機制

#### 🔧 新增腳本和工具
- `npm run collect:test` - 測試模式（50支/市場）
- `npm run collect:quick` - 快速模式（500支/市場）
- `npm run collect:full` - 完整模式（全部股票）
- `npm run collect:custom` - 自定義模式（指定數量）
- `npm run screener:test` - 保守模式掃描器測試

#### 🐛 重大修復
- **JSON檔案損壞**: 修復US和TW市場資料檔案
- **產業信息更新**: 改進錯誤處理和備份機制
- **API連接問題**: 優化速率限制和錯誤重試
