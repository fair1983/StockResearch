...
function determineMarket(symbol) {
  // 期權代碼（美股期權）
  if (OPTION_SYMBOL_REGEX.test(symbol)) {
    return 'US';
  }
...
function determineCategory(symbol, name = '') {
  ...
  if (/^[A-Z][A-Z0-9.]{0,6}$/.test(symbol)) {
    ...
    // 根據名稱判斷期權
    if (nm.includes('call') || nm.includes('put') || nm.includes('option')) {
      return 'option';
    }
    return 'stock';
  }
...
allStocks.forEach(stock => {
  ...
  if (!reorganizedStocks[targetMarket]) {
    reorganizedStocks[targetMarket] = { stocks: [], etfs: [], options: [] };
  }
  
  if (stock.category === 'etf') {
    reorganizedStocks[targetMarket].etfs.push(stock);
  } else if (stock.category === 'option') {
    reorganizedStocks[targetMarket].options.push(stock);
  } else {
...
Object.keys(reorganizedStocks).forEach(market => {
  ...
  if (marketData.options) {
    ...
    marketData.options = Array.from(optionMap.values()).sort((a, b) => a.symbol.localeCompare(b.symbol));
  }
});
