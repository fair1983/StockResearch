  ...
  try {
    ...
    for (const market of markets) {
      ...
      try {
        ...
        console.error(`❌ ${market} 市場數據收集失敗:`, error instanceof Error ? error.message : error);
      }
 ...
 * 收集單個市場數據
 */
async function collectMarketData(market: string) {
  ...
  try {
    ...
    await collector.collectMarketData(market as any);
    console.log(`✅ ${market} 市場數據收集完成`);
  } catch (error) {
 ...
 * 收集單個股票數據
 */
async function collectStockData(symbol: string, market: string) {
  ...
  try {
    ...
    // 獲取報價數據
    const quoteData = await collector.getQuote(symbol, market as any);
    if (quoteData) {
    ...
    const historicalData = await collector.getHistoricalData(symbol, market as any, period1, period2);
    if (historicalData) {
      ...
      try {
        ...
        if (symbols.length > 0) {
          ...
          for (const symbol of sampleSymbols) {
            const isStale = await collector.isDataStale(symbol, market as any);
            const status = isStale ? '過期' : '最新';
            console.log(`  - ${symbol}: ${status}`);
          }
        }
      } catch (error) {
        console.log(`  - 無法讀取 ${market} 市場狀態: ${error instanceof Error ? error.message : error}`);
      }
    }
...
switch (command) {
  ...
  case 'stock':
    ...
    if (!symbol || !stockMarket) {
      console.error('請指定股票代碼和市場 (例如: npx ts-node collect-yahoo-finance-data.ts stock AAPL US)');
      process.exit(1);
    }
  ...
  default:
    ...
    console.log('');
    console.log('使用方法:');
    console.log('  npx ts-node collect-yahoo-finance-data.ts all                    # 收集所有市場數據');
    console.log('  npx ts-node collect-yahoo-finance-data.ts market <MARKET>       # 收集指定市場數據');
    console.log('  npx ts-node collect-yahoo-finance-data.ts stock <SYMBOL> <MARKET> # 收集指定股票數據');
    console.log('  npx ts-node collect-yahoo-finance-data.ts status                # 檢查數據狀態');
    console.log('');
    console.log('市場代碼:');
    console.log('  US - 美國股市');
    console.log('  TW - 台灣股市');
    ...
    console.log('');
    console.log('範例:');
    console.log('  npx ts-node collect-yahoo-finance-data.ts all');
    console.log('  npx ts-node collect-yahoo-finance-data.ts market US');
    console.log('  npx ts-node collect-yahoo-finance-data.ts stock AAPL US');
    console.log('  npx ts-node collect-yahoo-finance-data.ts stock 2330.TW TW');
    break;
}
