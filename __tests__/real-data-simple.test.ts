...
describe('真實資料測試 (簡化版)', () => {
  ...
  describe('效能測試', () => {
    it('應該在合理時間內完成台積電技術指標計算', async () => {
      const tsmcData = testData.find(stock => stock.symbol === '2330.TW');
      
      const startTime = Date.now();
      
      await indicatorsCache.calculateAndCacheIndicators('TW', '2330', '1d', tsmcData.data);
      ...
      console.log(`台積電技術指標計算時間: ${executionTime}ms`);
    });

    it('應該在合理時間內完成特斯拉技術指標計算', async () => {
      const teslaData = testData.find(stock => stock.symbol === 'TSLA');
      
      const startTime = Date.now();
      
      await indicatorsCache.calculateAndCacheIndicators('US', 'TSLA', '1d', teslaData.data);
      ...
      console.log(`特斯拉技術指標計算時間: ${executionTime}ms`);
    });
  });
});
